...
const Pill: React.FC<{ text: string; variant?: string }> = ({ text, variant='light' }) => (
...
export default function LandDetailPage() {
  ...
  const [land, setLand] = useState<(Land & { parcel_no?: string }) | null>(null);
  ...
  const parcelFeature: Feature<Geometry, GeoJsonProperties> | null = useMemo(
    () => geo ? ({ type:'Feature', geometry: geo, properties:{} }) : null,
    [geo]
  );

  if (err) return <p className="text-center text-danger my-5">{err}</p>;
  ...
  return (
    <>
      ...
      <Container fluid className="pt-5 px-0 d-flex justify-content-center">
        <Card className="shadow-sm overflow-hidden" style={{ width:'min(92%, 840px)', borderRadius:'1rem', marginTop:'1rem' }}>
        <MapThumb
          ...
          showMeasurements
          showNeighbors
          neighborsParcelNo={land.parcel_no}   // must be defined by your API
          heightPx={300}
        />
      ...
      <Container className="my-5 px-3 px-md-4 px-lg-5">
        <Row className="g-4">
          <Col lg={8}>
            ...
            <p className="fs-5">{land.description?.trim() || 'No description provided.'}</p>
          </Col>
        ...
        <MapThumb
          ...
          movable
          showNeighbors
          neighborsParcelNo={land.parcel_no}
        />
      </Container>
    </>
  );
