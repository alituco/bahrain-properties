...
const PropertyNotesCard:React.FC<Props>=({propertyId,isAdmin,userFirstName,userLastName})=>{
  ...
  useEffect(()=>{
    (async()=>{
      const r=await fetch(`${API}/property-notes?parcel_no=${propertyId}`,{credentials:"include"});
  ...
  async function add(){
    ...
    const r=await fetch(`${API}/property-notes`,{
      ...
      body:JSON.stringify({parcel_no:propertyId,note_text:text})
    });
