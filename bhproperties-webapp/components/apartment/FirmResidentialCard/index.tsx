...
interface Record {
  ...
  updated_at:string; listing_type:'sale'|'rent';
}

interface Img { id:number; url:string; }

const FirmResidentialCard:React.FC<Props>=({propertyId})=>{
  ...
  const delImg = async (imgId:number)=>{
    if(!rec) return;
    const { isConfirmed } = await Swal.fire({title:'Delete this photo?',showCancelButton:true,icon:'warning'});
              ...
              <Row className="g-3 mb-3">
                <Col sm={6}>
                  ...
                  <span className="fw-medium">
                    {price ? `${Number(price).toLocaleString()} BHD${rec.listing_type==='rent'?'/mo':''}` : '—'}
                  </span>
                </Col>
              </Row>
              ...
              <Row className="g-2 mb-4">
                {imgs.map(img=>(
                  <Col xs={4} sm={3} md={2} key={img.id} className="thumb-wrapper">
                    <img src={img.url} className="img-fluid rounded border thumb-img"
                         alt="" onClick={()=>setPreview(img.url)}/>
                    <button className="btn btn-danger btn-sm thumb-close w-100 px-2" onClick={()=>delImg(img.id)}>
                      ×
                    </button>
                  </Col>
                ))}
                ...
                {imgs.length<MAX_PHOTOS && (
                  <Col xs={4} sm={3} md={2}>
                    <Form.Label htmlFor="imgUploader"
                    ...
                    <Form.Control id="imgUploader" type="file" multiple accept="image/*"
                                  onChange={upload} className="d-none"/>
                  </Col>
                )}
              </Row>
        ...
        <Modal.Body>
          ...
          <Form.Group className="mb-3">
            ...
            <Form.Control as="textarea" rows={4}
                          value={descr} onChange={e=>setDescr(e.target.value)} disabled={saving}/>
          </Form.Group>
...
export default FirmResidentialCard;
