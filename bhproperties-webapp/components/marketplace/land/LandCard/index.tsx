...
const LandCard: React.FC<{ land: Land }> = ({ land }) => {
  let parcelFeature: Feature<Geometry, GeoJsonProperties> | undefined;
  try {
    if (land.geojson && land.geojson.trim().startsWith('{')) {
  ...
  const fmtArea = `${Math.round(land.shape_area || 0).toLocaleString()} m²`;
  ...
  return (
    <Link href={href} className="text-reset text-decoration-none">
      <Card className="h-100 shadow-sm">
        <div className="position-relative border-bottom border-light-subtle">
          ...
          {land.asking_price != null && (
            <span
              ...
              style={{ backgroundColor: '#f4f1f1', color: '#d12c2c', opacity: 0.9, borderRadius: '0.7rem' }}
            >
        ...
        <Card.Body>
          <h6 className="fw-semibold mb-2">{land.title?.trim() || 'Untitled Land'}</h6>
